...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        #start = (self.startingPosition, corners)
        stateStart = self.startingPosition, []
        return stateStart
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            currentPosition, cornersV = state
            x, y = currentPosition
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            ...
            if not hitsWall:
                ...
                cornersVSuc= list (cornersV)
                if nextPosition in self.corners:
                    if nextPosition not in cornersVSuc:
                        cornersVSuc.append(nextPosition)
                nextState = nextPosition, cornersVSuc
                cost = 1
                successors.append((nextState, action, cost))
